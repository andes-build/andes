...
const wrapperPath = path.join(projectDir, 'config', 'scripts', 'andes-dev.mjs')

describe('andes-dev package bin', () => {
  it('uses a Node entrypoint for cross-platform package installs', () => {
    expect(packageJson.bin['andes-dev']).toBe('./config/scripts/andes-dev.mjs')
    expect(readFileSync(wrapperPath, 'utf8')).toMatch(/^#!\/usr\/bin\/env node\n/)
  })

  it('runs the dev CLI through Node without requiring Bash', () => {
    const root = mkdtempSync(path.join(tmpdir(), 'andes-dev-bin-'))
