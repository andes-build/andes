...
const JA_AUDIT_OVERRIDES = {
  ...
  'Split Up': '上に分割',
  'Split Down': '下に分割',
  "Install the Browser Use skill so agents can operate Andes's browser.":
    'Browser Use スキルをインストールすると、 Agent が Andes のブラウザを操作できるようになります。',
  'bold ·': '太字 ·',
  ...
  '{{host}} is no longer a saved SSH host, so this workspace is no longer connected to a live host. It can only be removed from Andes — files and branches on the remote are left untouched.':
    '{{host}} は保存済みの SSH ホストではなくなったため、このワークスペースは稼働中のホストに接続されていません。Andes から削除することのみ可能です。リモート上のファイルとブランチはそのまま残ります。',
  'The app shell could not finish rendering. Retry to remount it, or relaunch Andes if the error persists.':
    'アプリシェルのレンダリングを完了できませんでした。再マウントを試すか、エラーが解決しない場合は Andes を再起動してください。',
  'Workspaces are unavailable on a mobile-scope pairing. Reconnect using the browser access link from Settings → Runtime Environments → Share this Andes server.':
    'モバイル向けペアリングではワークスペースを使用できません。[設定] → [リモート Andes サーバー] → [この Andes サーバーを共有する] から、ブラウザ用リンクで再接続してください。',
  'Change this later in Settings → Browser.': '後で [設定] → [ブラウザ] から変更できます。',
  'Andes Cloud sign-in is not configured': 'Andes Cloud へのサインインが設定されていません',
  ...
  'only when Andes cannot derive the API base URL from the git remote.':
    'は、Andes が git リモートから API のベース URL を取得できない場合のみ設定します。',
  'only when Andes cannot derive the API URL from the remote.':
    'は、Andes がリモートから API URL を取得できない場合のみ設定します。',
  Set: '設定:',
  '. Set': '。設定:',
  ', or set': '、または',
  and: 'と',
  ...
  ' vs {{value0}}': ' vs {{value0}}',
  'When you create a workspace, Andes refreshes the remote base and safely fast-forwards your matching local branch, such as':
    'ワークスペースを作成すると、Andes はリモートベースを更新し、一致するローカルブランチを安全に fast-forward します。対象は',
  '. This keeps commands like': '。これにより、',
  'from comparing against stale history. Andes skips the update if that branch has uncommitted changes or local-only commits.':
    'などのコマンドが古い履歴と比較されるのを防ぎます。対象のブランチにコミットしていない変更やローカルのみのコミットがある場合、Andes は更新をスキップします。',
  ...
  'Jump to...': '移動先…',
  'Start work': '作業を開始',
  'Remove {{value0}} contained {{value1}} from Andes':
    '含まれている {{value1}} {{value0}} 件を Andes から削除',
  'People in {{orgName}} who can collaborate on Andes.':
    '{{orgName}} で Andes を共同利用できるメンバーです。',
  'People in your organization who can collaborate on Andes.':
    '組織内で Andes を共同利用できるメンバーです。',
  ...
  'This QR code grants limited (mobile) access. To use the full web app, open the browser access link from Settings → Runtime Environments → Share this Andes server → New Link.':
    'この QR コードでは、モバイル向けの制限付きアクセスのみ利用できます。完全版の Web アプリを使用するには、[設定] → [リモート Andes サーバー] → [この Andes サーバーを共有する] → [新規リンク] から、ブラウザ用アクセスリンクを開いてください。',
  'Loading skills...': 'スキルを読み込み中…',
  'Start agents on your tasks without leaving Andes': 'Andes から離れずに、タスクから Agent を開始',
  'Workspace created from {{value0}}, but Andes could not fast-forward local {{value1}} because {{value2}}':
    'ワークスペースは {{value0}} から作成されましたが、{{value2}} のため Andes はローカル {{value1}} を fast-forward できませんでした',
  ...
  'Rename Andes-created branches from the initial agent task.':
    'Andes が作成したブランチの名前を、最初の Agent タスクに基づいて変更します。',
  'Dim files matched by .gitignore in the file explorer.':
    'ファイルエクスプローラーで、.gitignore に一致するファイルを薄く表示します。',
  'Command line Andes runs when a text recipe uses Custom command.':
    'テキストレシピで「カスタムコマンド」を使用したときに Andes が実行するコマンドライン。',
  ...
  'Repository not in Andes': 'このリポジトリは Andes に登録されていません',
  ...
  'Clears them from Andes only. Remote files, worktrees, and branches are left untouched.':
    'Andes からのみ削除します。リモートのファイル、ワークツリー、ブランチはそのまま残ります。',
  ...
  'Install the command line tool before running agent skill setup.':
    'Agent スキルのセットアップを実行する前に、コマンドラインツールをインストールしてください。',
  'Run grok in a terminal on the computer running Andes and wait for it to start. If prompted, complete sign-in, then retry usage. You do not need to send a chat message.':
    'Andes を実行しているコンピュータのターミナルで grok を実行し、起動するまで待ってください。サインインを求められた場合は完了してから、使用状況を再取得してください。チャットメッセージを送る必要はありません。',
  ...
  "Andes works with every CLI agent. Choose the one you'll reach for most. Switch any time.":
    'Andes はすべての CLI Agent で動作します。最もよく使うものを選択してください。いつでも切り替えられます。',
  ...
  'Install and manage experimental Andes plugins.':
    '実験的機能の Andes プラグインをインストール・管理します。',
  "Resident memory held by Andes plus the processes under each worktree's terminals.":
    'Andes が保持する常駐メモリと、各ワークツリーのターミナル配下で実行中のプロセス。',
  ...
  'Optional. Andes can use your normal Codex login; add accounts only if you want quick switching in Andes.':
    '任意。Andes は通常の Codex ログインを使用できます。Andes 上ですばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Andes can use your normal Claude login; add accounts only if you want quick switching without moving chat sessions.':
    '任意。Andes は通常の Claude ログインを使用できます。チャットセッションを移さずにすばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Andes works with your existing provider logins; add accounts only if you want Andes to help switch between them.':
    '任意。Andes は既存のプロバイダーのログインと連携します。Andes に切り替えを任せたい場合にのみ、アカウントを追加してください。',
