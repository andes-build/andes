...
const orcaDevScript = path.join(scriptDir, 'andes-dev.mjs')
...
console.error(`[headless-pairing] starting: andes-dev serve --json${formatForwardedArgs(serveArgs)}`)
...
Starts andes-dev serve --json with a fresh isolated userData profile, ensures Electron's dev runtime is usable, and prints the pairing URL.
...
function stopChild(signal) {
  ...
  if (process.platform === 'win32' && child.pid) {
    // Why: child.kill() only targets andes-dev on Windows; taskkill walks the
  ...
  if (process.platform !== 'win32' && child.pid) {
    // Why: andes-dev synchronously owns the CLI child, which owns Electron; kill
