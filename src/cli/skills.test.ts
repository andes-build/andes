...
describe('orca skills CLI', () => {
  ...
  it('lists installable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: andes skills install --skill <name> [--skill <name> ...]',
        '   or: andes skills install --all',
        ''
      ].join('\n')
    )
      ...
      JSON.stringify(
        {
          ...
          error: {
            ...
            message:
              "andes skills install --json only supports --dry-run. Real installs stream npx's " +
  ...
  it('prints the resolved install command without running it for --dry-run', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      'npx --yes skills add https://github.com/andes-build/andes --skill alpha --global --agent claude-code --agent universal -y\n\n' +
  ...
  it('drops --global for --local in the dry-run command and JSON', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      'npx --yes skills add https://github.com/andes-build/andes --skill alpha --agent claude-code --agent universal -y\n\n' +
        'Rerun without --dry-run to install now.\n'
    )
  ...
  it('lists updatable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: andes skills update --skill <name> [--skill <name> ...]',
        '   or: andes skills update --all',
        ''
      ].join('\n')
    )
    expect(spawnMock).not.toHaveBeenCalled()
  ...
  it('reports the command it is about to run on stderr', async () => {
    ...
    expect(stderrSpy).toHaveBeenCalledWith(
      'Running: npx --yes skills add https://github.com/andes-build/andes --skill alpha --global --agent claude-code --agent universal -y\n'
    )
  })
      ...
      JSON.stringify(
        {
          ...
          error: {
            code: 'invalid_argument',
            message:
              "andes skills update --json only supports --dry-run. Real updates stream npx's " +
              "own output, which isn't JSON."
          },
          _meta: { runtimeId: null }
        },
