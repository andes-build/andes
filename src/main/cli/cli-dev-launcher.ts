...
export async function ensureDevLauncher(args: {
  ...
  if (args.commandName === DEV_COMMAND_NAME && args.platform !== 'win32') {
    // Why: dev PTYs prepend this dir to PATH, so keep a local `andes` alias without claiming the global command.
    await writeFile(join(dirname(launcherPath), 'andes'), content, {
      encoding: 'utf8',
      mode: 0o755
    })
  }
