...
describe('runtime RPC startup failure reporting', () => {
  ...
  it.each([
    ['EACCES', "Check permissions on Andes's data folder"],
    ['EPERM', "Check permissions on Andes's data folder"],
    ...
    ['ENOENT', "Andes's data folder may be missing"],
  ...
  it('falls back to a plain restart when the cause is unrecognised', async () => {
    ...
    const detail = showMessageBoxMock.mock.calls[0]?.[1]?.detail as string
    expect(detail).toContain('Restart Andes to try again.')
    expect(detail).not.toContain("Check permissions on Andes's data folder")
  })
