...
> = {
  permission_denied: {
    ...
    fallback:
      "Andes couldn't write its runtime file. Check permissions on Andes's data folder, then restart."
  },
  storage_unavailable: {
    key: 'runtimeRpc.startupFailure.guidance.storageUnavailable',
    fallback: 'Your disk may be full or read-only. Free up space, then restart Andes.'
  },
  invalid_path: {
    key: 'runtimeRpc.startupFailure.guidance.invalidPath',
    fallback:
      "Andes's data folder may be missing, moved, or at a path that is too long. Restore it or use a shorter path, then restart Andes."
  },
  address_in_use: {
    key: 'runtimeRpc.startupFailure.guidance.addressInUse',
    fallback: 'Another process may be holding the port. Restart Andes to try again.'
  },
  unknown: {
    key: 'runtimeRpc.startupFailure.guidance.unknown',
    fallback: 'Restart Andes to try again.'
  }
}
...
function createRuntimeRpcStartupFailureDialogOptions(error: unknown): MessageBoxOptions {
  ...
  return {
    ...
    cancelId: 0,
    noLink: true,
    title: translateMain('runtimeRpc.startupFailure.title', 'Command line tool unavailable'),
    message: translateMain(
      'runtimeRpc.startupFailure.message',
      "Andes couldn't start its local command transport."
    ),
    detail: translateMain(
      'runtimeRpc.startupFailure.detail',
      'Andes will continue to work, but commands such as andes status, andes terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}',
