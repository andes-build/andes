...
const globallyUpdatableNames = new Set(['computer-use', 'andes-cli', 'orchestration'])
...
describe('skill freshness name-scoped update eligibility', () => {
  ...
  it.each([
    ...
    (status, topology) => {
      ...
      expect(
        eligible([
          placement('andes-cli'),
          placement('andes-cli', { id: `outlier-${status}-${topology}`, status, topology })
        ])
      ).toEqual(['andes-cli'])
    }
  )
  ...
  it.each(['unrecognized', 'inaccessible', 'newer-known'] as const)(
    ...
    (status) => {
      ...
      expect(
        eligible([
          placement('andes-cli', { id: 'blocked-canonical', status }),
          placement('andes-cli', {
            id: 'andes-cli-claude',
            rootId: 'home-claude',
            topology: 'provider-alias',
    ...
    expect(
      eligible([
        ...
        placement('andes-cli', {
          id: 'andes-cli-gemini',
          rootId: 'home-gemini',
          unresolvedPath: '/home/.gemini/skills/andes-cli',
          resolvedPath: '/home/.gemini/skills/andes-cli',
  ...
  it('does not offer an official canonical copy missing from the updater lock (#10791)', () => {
    expect(eligibleSkillUpdateNames([placement('andes-cli')], new Set())).toEqual([])
  })

  it('builds only an explicit, deterministic global command', () => {
    expect(buildTargetedSkillUpdateCommand(['orchestration', 'andes-cli', 'andes-cli'])).toBe(
      'npx skills update andes-cli orchestration --global'
    )
    expect(buildTargetedSkillUpdateCommand([])).toBeNull()
    expect(buildTargetedSkillUpdateCommand(['andes-cli;echo unsafe'])).toBeNull()
  })
})
