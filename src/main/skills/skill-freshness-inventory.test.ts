...
async function fixture() {
  ...
  await Promise.all([
        ...
        `${JSON.stringify({
          ...
          skills: {
            'andes-cli': {
              ...
              skillPath: 'skills/andes-cli/SKILL.md',
    ...
    writeFile(
      ...
      `${JSON.stringify({ schemaVersion: 1, skills: { 'andes-cli': snapshots } }, null, 2)}\n`
    ),
    writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          releases: [
            ...
            { appVersion: '2.0.0', skills: { 'andes-cli': 2 } },
            { appVersion: '3.0.0', skills: { 'andes-cli': 3 } }
          ]
        },
        null,
        2
  ...
  const writeSkill = async (rootPath: string, markdown: string): Promise<string> => {
    const directory = join(rootPath, 'andes-cli')
...
describe('read-only skill freshness inventory', () => {
  it('offers an exact older official name only when all global placements are safe', async () => {
    ...
    expect(inventory.eligibleUpdateNames).toEqual(['andes-cli'])
  })
  ...
  it('trusts the updater lock for canonical bytes the bundle has never seen (#11220 scan half)', async () => {
    ...
    expect(getSkillFreshnessDisplayStatus(inventory, 'andes-cli')).toBe('up-to-date')
  })
  ...
  it('scopes the lock hash to the current bundle, not every path a revision ever shipped', async () => {
    ...
    registry.skills['andes-cli'][0].files.push(legacy)
  ...
  it('still flags canonical bytes that do not match what the lock says was installed', async () => {
    ...
    expect(getSkillFreshnessDisplayStatus(inventory, 'andes-cli')).toBe('needs-attention')
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      await symlink(canonical, join(claudeRoot, 'andes-cli'))
    ...
    async () => {
      ...
      const repos = await Promise.all(
        ['one', 'two'].map(async (id) => {
          ...
          await symlink(shared, join(root, 'andes-cli'))
  ...
  it('keeps an unreadable foreign-home placement visible without withholding the update', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.oldMarkdown)
    const inaccessiblePath = join(test.homeDir, '.codex', 'skills', 'andes-cli')
  ...
  it('does not lose an inaccessible known repository placement', async () => {
    ...
    const inaccessiblePath = join(repoPath, '.agents', 'skills', 'andes-cli')
  ...
  it('does not classify an empty plugin-cache directory as a skill', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.currentMarkdown)
    const emptyRoot = join(test.homeDir, '.codex', 'plugins', 'cache', 'vendor', 'andes-cli')
  ...
  it('classifies exact current bytes as current when a later snapshot reuses them', async () => {
    ...
    const registry = JSON.parse(await readFile(registryPath, 'utf8'))
    registry.skills['andes-cli'].push(snapshot(4, test.currentMarkdown))
    await writeFile(registryPath, `${JSON.stringify(registry, null, 2)}\n`)
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.currentMarkdown)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(inventory.installations).toEqual([
        expect.objectContaining({ name: 'andes-cli', status: 'current', topology: 'canonical-copy' })
      ])
    },
    90_000
  )
