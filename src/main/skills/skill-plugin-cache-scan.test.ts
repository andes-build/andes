...
describe('plugin skill candidate scan', () => {
  it('stops at the package candidate budget and marks the scan incomplete', async () => {
    ...
    await Promise.all(
      ['one', 'two'].map(async (vendor) => {
        await mkdir(join(root, vendor, 'andes-cli'), { recursive: true })
        await writeFile(join(root, vendor, 'andes-cli', 'SKILL.md'), '# Orca CLI\n')
      })
    )

    const result = await scanKnownPluginSkillCandidates(root, new Set(['andes-cli']), {
      maximumCandidates: 1
    })
  ...
  it('stops at the entry budget and reports the truncation at the scan root', async () => {
    ...
    expect(result.candidates).toEqual([{ name: 'andes-cli', path: join(root, 'one', 'andes-cli') }])
  ...
  async function createManifestWithMissingSkillRoots(): Promise<{
    ...
    const candidate = join(packageRoot, 'a-skills', 'andes-cli')
  ...
  it('stops at the entry budget while resolving declared skill roots', async () => {
    ...
    expect(result.candidates).toEqual([{ name: 'andes-cli', path: candidate }])
    expect(result.issues).toEqual([{ path: root, reason: 'entry-limit', errorCode: null }])
  })
  ...
  it('completes a real-shaped Codex cache without reporting coverage issues', async () => {
    ...
    const skill = join(packageRoot, 'skills', 'andes-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(
    ...
    expect(result).toEqual({ candidates: [{ name: 'andes-cli', path: skill }], issues: [] })
  })

  it('does not emit a plugin directory that only shares a skill name', async () => {
    // The cached plugin is itself called andes-cli. Only the SKILL.md below it is a skill.
    const root = await mkdtemp(join(tmpdir(), 'orca-plugin-name-collision-'))
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'openai-bundled', 'andes-cli', '1.0.0')
    const skill = join(packageRoot, 'skills', 'andes-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
  ...
  it('does not emit a bare known-name directory that carries no SKILL.md', async () => {
    ...
    temporaryDirectories.push(root)
    await mkdir(join(root, 'vendor', 'andes-cli', 'assets'), { recursive: true })
  ...
  it('stops descending once a skill package payload exceeds the nested skill budget', async () => {
    ...
    const buried = join(skill, 'templates', 'starter', 'examples', 'andes-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
  ...
  it.each([
    ...
    async (intermediateDepth, expectFound) => {
      ...
      const candidate = join(parent, 'andes-cli')
      await mkdir(candidate, { recursive: true })
      await writeFile(join(candidate, 'SKILL.md'), '# Orca CLI\n')

      const result = await scanKnownPluginSkillCandidates(root, new Set(['andes-cli']))

      expect(result.candidates).toEqual(expectFound ? [{ name: 'andes-cli', path: candidate }] : [])
  ...
  it('reports a depth-truncated subtree as scan coverage instead of a skill candidate', async () => {
    ...
    const hiddenSkill = join(root, ...segments, 'andes-cli')
  ...
  it('scans only declared Codex plugin skill roots', async () => {
    ...
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'custom-skills', 'group', 'andes-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(candidate, { recursive: true })
    await mkdir(
    ...
    expect(result).toEqual({
      candidates: [{ name: 'andes-cli', path: candidate }],
      issues: []
    })
  })
  ...
  it('uses the default skills root for compatible manifests without a skills field', async () => {
    ...
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'skills', 'nested', 'andes-cli')
  ...
  it('falls back to traversal when a manifest skills path is invalid', async () => {
    ...
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'custom-skills', 'andes-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(candidate, { recursive: true })
    await writeFile(
      join(packageRoot, '.codex-plugin', 'plugin.json'),
  ...
  it('discovers nested skill packages recursively within declared roots', async () => {
    ...
    await mkdir(join(packageRoot, '.claude-plugin'), { recursive: true })
    await mkdir(join(skillRoot, 'templates', 'andes-cli'), { recursive: true })
    ...
    await writeFile(join(skillRoot, 'templates', 'andes-cli', 'SKILL.md'), '# Orca CLI\n')

    const result = await scanKnownPluginSkillCandidates(root, new Set(['andes-cli']))

    expect(result).toEqual({
      candidates: [{ name: 'andes-cli', path: join(skillRoot, 'templates', 'andes-cli') }],
      issues: []
    })
  })
  ...
  it('rejects Windows parent traversal without hiding the default skills root', async () => {
    ...
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const candidate = join(packageRoot, 'skills', 'andes-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(candidate, { recursive: true })
    await writeFile(
      join(packageRoot, '.codex-plugin', 'plugin.json'),
  ...
  it('bounds how many skill roots one manifest can declare', async () => {
    ...
    const candidate = join(packageRoot, 'r0000', 'andes-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    await mkdir(candidate, { recursive: true })
    await writeFile(join(candidate, 'SKILL.md'), '# Orca CLI\n')
    await writeFile(
  ...
  it('keeps valid roots when a skills array contains an invalid value', async () => {
    ...
    temporaryDirectories.push(root)
    const packageRoot = join(root, 'vendor', 'plugin', '1.0.0')
    const defaultCandidate = join(packageRoot, 'skills', 'andes-cli')
    const declaredCandidate = join(packageRoot, 'custom-skills', 'andes-cli')
    await mkdir(join(packageRoot, '.codex-plugin'), { recursive: true })
    ...
    expect(result).toEqual({
      candidates: [{ name: 'andes-cli', path: declaredCandidate }],
      issues: []
    })
  })
  ...
  it('does not reset the depth budget across nested plugin manifests', async () => {
    ...
    const hiddenCandidate = join(pluginRoot, 'andes-cli')
  ...
  it('keeps scanning past the issue budget', async () => {
    ...
    temporaryDirectories.push(root)
    const candidate = join(root, 'zz-package', 'skills', 'andes-cli')
    await Promise.all(
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      await mkdir(join(outside, 'andes-cli'), { recursive: true })
      await mkdir(root, { recursive: true })
      await writeFile(join(outside, 'andes-cli', 'SKILL.md'), '# Orca CLI\n')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      temporaryDirectories.push(parent)
      const root = join(parent, 'cache')
      const skill = join(root, 'vendor', 'andes-cli')
