...
describe('skillUpdateFailedNames', () => {
  it('treats a convergent copy that is now current as landed', () => {
    expect(
      skillUpdateFailedNames(['andes-cli'], [placement('andes-cli', 'current')], noLocks)
    ).toEqual([])
  })

  it('reports a copy the run left outdated', () => {
    expect(
      skillUpdateFailedNames(['andes-cli'], [placement('andes-cli', 'outdated')], noLocks)
    ).toEqual(['andes-cli'])
  })
  ...
  it('reports a half-written bundle instead of reading it as success', () => {
    ...
    expect(
      skillUpdateFailedNames(['andes-cli'], [placement('andes-cli', 'unrecognized')], noLocks)
    ).toEqual(['andes-cli'])
  })

  it('reports an unreadable copy', () => {
    expect(
      skillUpdateFailedNames(['andes-cli'], [placement('andes-cli', 'inaccessible')], noLocks)
    ).toEqual(['andes-cli'])
  })

  it('reports a skill the run removed outright', () => {
    expect(skillUpdateFailedNames(['andes-cli'], [], noLocks)).toEqual(['andes-cli'])
  })
  ...
  it('accepts a revision newer than this build ships', () => {
    ...
    expect(
      skillUpdateFailedNames(['andes-cli'], [placement('andes-cli', 'newer-known')], noLocks)
    ).toEqual([])
  })
  ...
  it('ignores placements the update command never writes to', () => {
    expect(
      skillUpdateFailedNames(
        ['andes-cli'],
        [placement('andes-cli', 'current'), placement('andes-cli', 'outdated', 'plugin-cache')],
        noLocks
      )
    ).toEqual([])
  })
  ...
  it('fails the name when any convergent alias was left behind', () => {
    expect(
      skillUpdateFailedNames(
        ['andes-cli'],
        [placement('andes-cli', 'current'), placement('andes-cli', 'outdated', 'provider-alias')],
        noLocks
      )
    ).toEqual(['andes-cli'])
  })
  ...
  it('judges each requested name independently', () => {
    expect(
      skillUpdateFailedNames(
        ['andes-cli', 'orchestration'],
        [placement('andes-cli', 'current'), placement('orchestration', 'outdated')],
        noLocks
      )
  ...
  it('treats unrecognized content whose tree sha matches the lock as landed', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['andes-cli'],
        [placement('andes-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle')],
        new Map([['andes-cli', 'ahead-of-bundle']])
      )
    ).toEqual([])
  })
  ...
  it('still reports unrecognized content whose bytes do not match the lock', () => {
    expect(
      skillUpdateFailedNames(
        ['andes-cli'],
        [placement('andes-cli', 'unrecognized', 'canonical-copy', 'half-written-bytes')],
        new Map([['andes-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['andes-cli'])
  })
  ...
  it('never forgives an outdated copy, even at the lock hash', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['andes-cli'],
        [placement('andes-cli', 'outdated', 'canonical-copy', 'locked-revision')],
        new Map([['andes-cli', 'locked-revision']])
      )
    ).toEqual(['andes-cli'])
  })
  ...
  it('does not let a lock-matching canonical copy excuse a degraded alias', () => {
    expect(
      skillUpdateFailedNames(
        ...
        [
          placement('andes-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle'),
          placement('andes-cli', 'inaccessible', 'provider-alias')
        ],
        new Map([['andes-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['andes-cli'])
...
describe('skillUpdateFailedNames over a real inventory', () => {
  ...
  async function postCutFixture(): Promise<{ homeDir: string; installedTreeSha: string }> {
    ...
    const skillDir = join(homeDir, '.agents', 'skills', 'andes-cli')
    ...
    const current = await readFile(join(repoRoot, 'skills', 'andes-cli', 'SKILL.md'))
    await writeFile(
  ...
  it('accepts post-cut source content when the lock records exactly those bytes', async () => {
    ...
    const canonical = inventory.installations.filter(
      (entry) => entry.name === 'andes-cli' && entry.topology === 'canonical-copy'
    )
    expect(canonical).toHaveLength(1)
    expect(canonical[0].status).toBe('newer-known')
    expect(canonical[0].installedReleaseRevision).toBeNull()

    expect(skillUpdateFailedNames(['andes-cli'], inventory.installations, locks)).toEqual([])
  })
