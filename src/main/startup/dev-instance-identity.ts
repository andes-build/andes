...
const BASE_APP_NAME = 'Andes'
const BASE_APP_USER_MODEL_ID = 'build.andes'
...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'Andes'.
  appName: string
}
 ...
 * CFBundleName, which downstream forks ship differently ("Andes ALab Edition").
...
export function getDevInstanceIdentity(
  ...
  return {
    name: dockTitle,
    // Why: one stable Keychain key ('Andes Dev Safe Storage') for all dev
