...
import menuBarIconPath from '../../../resources/tray/andes-menu-barTemplate.png?asset&asarUnpack'
import menuBarIconRetinaPath from '../../../resources/tray/andes-menu-barTemplate@2x.png?asset&asarUnpack'
...
export type SystemTrayOptions = {
  ...
  /** Quit Andes for real (caller must set the quitting latch before quitting). */
  onQuit: () => void
}
...
function baseTooltip(): string {
  ...
  return devIndicator.label ? `Andes DEV (${devIndicator.label})` : 'Andes DEV'
}
      ...
      try {
        ...
        tray.setToolTip(
          devIndicator
            ...
            : translateMain('tray.activityWaiting', 'Andes - activity waiting')
        )
        return
      } catch (error) {
...
export function createSystemTray(opts: SystemTrayOptions): Tray | null {
  ...
  const menu = Menu.buildFromTemplate([
    ...(devIndicator
      ...
      : []),
    {
      label: translateMain('tray.openOrca', 'Open Andes'),
