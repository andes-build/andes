...
const ATOM_FEED_URL = 'https://github.com/andes-build/andes/releases.atom'
const RELEASES_DOWNLOAD_BASE = 'https://github.com/andes-build/andes/releases/download'
...
const TAG_HREF_RE = /href="https:\/\/github\.com\/andes-build\/andes\/releases\/tag\/([^"]+)"/g
...
async function getReleaseAssetReadiness(tag: string, assetName: string): Promise<ReleaseReadiness> {
  ...
  const isGitHubReleaseAsset =
    ...
    (isRelativeAsset ||
      /^https:\/\/github\.com\/andes-build\/andes\/releases\/download\//i.test(assetName))
