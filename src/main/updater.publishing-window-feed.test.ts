...
describe('updater', () => {
  ...
  it('repins the generic feed to the newest RC tag for a prerelease user', async () => {
    ...
    expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
      provider: 'generic',
      url: 'https://github.com/andes-build/andes/releases/latest/download'
    })
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/andes-build/andes/releases/download/v1.3.17-rc.2'
      })
  ...
  it('repins the generic feed to a newer stable tag for a prerelease user', async () => {
    ...
    await vi.waitFor(() => {
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/andes-build/andes/releases/download/v1.3.19'
      })
    })
    expect(autoUpdaterMock.allowPrerelease).not.toBe(true)
  })
  ...
  it('uses last-good concrete feed when a user-initiated check lands during publishing', async () => {
    ...
    expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
      provider: 'generic',
      url: 'https://github.com/andes-build/andes/releases/download/v1.4.26'
    })
    expect(autoUpdaterMock.setFeedURL).not.toHaveBeenCalledWith({
      provider: 'generic',
      url: 'https://github.com/andes-build/andes/releases/download/v1.4.27'
    })
    expect(autoUpdaterMock.setFeedURL.mock.calls.slice(feedCallsBeforeCheck)).not.toContainEqual([
      {
        provider: 'generic',
