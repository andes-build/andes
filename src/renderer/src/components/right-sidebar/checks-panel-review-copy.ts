...
export function concurrentLookupDetail(input: ChecksPanelReviewStateInput): string | undefined {
  ...
  if (input.reviewLookup === 'positive_unresolved') {
    return translate(
      ...
      'Andes also has saved {{reviewLabel}} information that it could not verify.',
      { reviewLabel }
    )
  }
  if (isRateLimitRefresh(refresh)) {
    return translate(
      ...
      'Andes also could not check {{reviewLabel}} status because {{provider}} is temporarily limiting requests.',
  ...
  if (refresh?.errorType === 'network') {
    return translate(
      ...
      'Andes also could not check {{reviewLabel}} status because this environment could not reach {{provider}}.',
      { reviewLabel, provider: providerName }
    )
  }
  if (refresh?.status === 'error' || isHardRefreshError(refresh)) {
    return translate(
      ...
      'Andes also could not confirm whether this branch already has a {{reviewLabel}}.',
      { reviewLabel }
    )
  }
  return undefined
...
export function transientRefreshState(
  ...
  if (refresh?.errorType === 'unknown') {
    return {
      ...
      description: translate(
        ...
        'The lookup failed, so Andes could not confirm whether this branch already has a {{reviewLabel}}.',
        { reviewLabel }
      )
    }
  }
  return {
    ...
    description: translate(
      ...
      'Andes could not confirm whether this branch already has a {{reviewLabel}}. Retry to check again.',
      { reviewLabel }
    )
  }
}
