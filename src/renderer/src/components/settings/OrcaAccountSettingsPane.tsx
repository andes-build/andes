...
function accountStatusCopy(
  ...
  if (state === 'unconfigured') {
    return translate(
      ...
      'Andes sign-in is unavailable in this build.'
    )
  }
  if (state === 'local') {
    return translate(
      'auto.components.settings.orcaAccount.signedOut',
      'Sign in to extend Andes with cloud features, including Artifacts and Andes Relay.'
    )
  }
            ...
            <div className="flex flex-wrap items-center gap-2">
              <p className="text-sm font-medium">
                {authStatus?.cloud?.displayName?.trim() ||
                  translate('auto.components.settings.orcaAccount.account', 'Andes account')}
              </p>
              {connected ? (
            ...
            >
              {connecting
                ...
                : authStatus?.state === 'reconnect-required'
                  ...
                  : translate('auto.components.settings.orcaAccount.signIn', 'Sign in to Andes')}
            </Button>
          )}
        </div>
            ...
            <div className="grid gap-5 md:grid-cols-2 md:gap-0 md:divide-x md:divide-border/60">
              <AccountBenefit
                ...
                description={translate(
                  ...
                  'Publish HTML and Markdown files, then manage every shared link from Andes.'
                )}
              />
              <AccountBenefit
                icon={Smartphone}
                className="md:pl-6"
                title={translate('auto.components.settings.orcaAccount.relayTitle', 'Andes Relay')}
                description={translate(
                  'auto.components.settings.orcaAccount.relayDescription',
                  'Connect Andes Mobile to this desktop across cellular or any Wi-Fi.'
                )}
              />
            </div>
