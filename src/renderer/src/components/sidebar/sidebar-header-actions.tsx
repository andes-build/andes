...
export function SidebarHeaderActions({
  ...
  // Spec 002, criterion 6: no worktree-creation button or repo filter in simple mode.
  const isSimpleMode = useInterfaceMode() === INTERFACE_MODE_SIMPLE
  const hideNewWorkspaceButton = isSimpleMode
  const effectiveHideWorkspaceOptions = hideWorkspaceOptions || isSimpleMode

  if (compact) {
    return (
      <div className="flex shrink-0 items-center gap-1">
        {hideNewWorkspaceButton ? null : (
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                variant="ghost"
        ...
        {effectiveHideWorkspaceOptions ? null : (
