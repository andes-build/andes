...
function eligibleInventory(): SkillFreshnessInventory {
  return {
    schemaVersion: 1,
    installations: [placement('andes-cli')],
    eligibleUpdateNames: ['andes-cli'],
    scanIssues: [],
    scannedAt: 1
  }
}
...
describe('SkillFreshnessUpdateDialog', () => {
  ...
  it('starts a background run with the eligible names', async () => {
    ...
    expect(skillsApi.startUpdateRun).toHaveBeenCalledWith(['andes-cli'])
  })

  it('shows indeterminate progress and says the run survives closing', async () => {
    await renderDialog()
    await openViaRequest()
    await emitRun({ state: 'running', names: ['andes-cli'], startedAt: 1, output: '' })
    ...
    expect(
      container?.querySelector('[data-skill-row="andes-cli"]')?.getAttribute('data-state-label')
    ).toBe('pending')
  })
  ...
  it('attributes failures to the names the re-scan says are still outdated', async () => {
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('andes-cli'), placement('orchestration')],
      eligibleUpdateNames: ['andes-cli', 'orchestration'],
      scanIssues: [],
      scannedAt: 1
    }
    await renderDialog()
  ...
  it('keeps the same row elements across the whole run instead of swapping layouts', async () => {
    await renderDialog()
    await openViaRequest()
    const before = container?.querySelector('[data-skill-row="andes-cli"]')
    expect(before?.getAttribute('data-state-label')).toBe('available')

    await emitRun({ state: 'running', names: ['andes-cli'], startedAt: 1, output: '' })
    const during = container?.querySelector('[data-skill-row="andes-cli"]')
    ...
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('andes-cli', { status: 'current', installedReleaseRevision: 2 })],
      eligibleUpdateNames: [],
      scanIssues: [],
      scannedAt: 5
    }
    await emitRun({ state: 'success', names: ['andes-cli'], finishedAt: 2, output: 'done' })
    await rerender()
    const after = container?.querySelector('[data-skill-row="andes-cli"]')
  ...
  it('collapses each skill’s locations behind its own disclosure', async () => {
    ...
    const row = container?.querySelector('[data-skill-row="andes-cli"]')
  ...
  it('shows the up-to-date state once every installation is current', async () => {
    ...
    expect(container?.textContent).toContain('All installed Andes skills are up to date.')
  ...
  it('raises no row for a skill whose only finding is a project-owned copy', async () => {
    ...
    // directory. Andes only ever updates global skills, so a "Skipped" row here asserts
  ...
  it('names the skill in the stale-record remedy so the command is runnable as-is', async () => {
    ...
    expect(container?.textContent).toContain(
      'npx skills add https://github.com/andes-build/andes --skill orchestration --global'
    )
  })
  ...
  it('keeps the stale-record remedy when a project copy is listed beside it', async () => {
    ...
    // runnable command with advice about a copy the user never asked Andes to update.
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
  ...
  it('stays coherent while an idle re-scan is in flight', async () => {
    ...
    expect(container?.textContent).toContain('Checking installed Andes skills…')
  ...
  it('surfaces a scan error instead of a stale summary', async () => {
    ...
    expect(container?.querySelector('[data-skill-row="andes-cli"]')).not.toBeNull()
    ...
    expect(container?.querySelector('[data-skill-row="andes-cli"]')).toBeNull()
    expect(findButton('Update 1 skill')).toBeUndefined()
  })
  ...
  it('shows incomplete plugin coverage without presenting a fabricated skill copy', async () => {
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
        placement('andes-cli', { status: 'current', observedPackageDigest: 'current' })
      ],
      eligibleUpdateNames: [],
      scanIssues: [
        {
    ...
    expect(container?.textContent).toContain(
      'Andes could not finish checking plugin-managed skills.'
    )
    ...
    expect(container?.textContent).not.toContain('All installed Andes skills are up to date.')
    ...
    expect(container?.textContent).not.toContain(
      '/home/.codex/plugins/cache/vendor/locked/andes-cli'
    )
  })
  ...
  // Why: Andes's own traversal bounds are not the user's to act on. Headlining them
