  ...
  switch (chip) {
    case 'newer':
      return translate(
        ...
        'This copy is a later version than the one this build of Andes ships, so Andes left it alone rather than roll it back. Updating Andes will bring the two back in line.'
      )
    case 'unrecognized':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonUnrecognized',
        'The copy here doesn’t match the official version — it may be modified, or a different skill with the same name. Andes left it out of the update so it won’t overwrite it. Remove it if you want Andes to update this skill.'
      )
    case 'read-only':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonReadOnly',
        'This copy is in a read-only location, so Andes left it out of the update. Change its permissions to let Andes update it.'
      )
    case 'inaccessible':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonInaccessible',
        'Andes couldn’t read this copy, so it left the skill out of the update.'
      )
    case 'in-a-repo':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonInRepo',
        'This is a project skill, not a global one — Andes only updates your global skills, so it left this out of the update.'
      )
    case 'plugin-cache':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonPluginCache',
        'A plugin manages this skill, so Andes left it out of the update — update the plugin instead.'
      )
    case 'external-link':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonExternalLink',
        'This copy is a shortcut pointing outside Andes’s skill folders, so Andes left it out of the update.'
      )
    case 'broken-link':
      return translate(
        'auto.components.skills.SkillFreshnessRow.skippedReasonBrokenLink',
        'This copy is a shortcut to something that no longer exists, so Andes left it out — you can safely delete it.'
      )
    case 'duplicate':
      return translate(
    ...
    case undefined:
      ...
      return skillName
        ...
        : translate(
            ...
            'Andes left this skill out of the update command.'
          )
  }
}
