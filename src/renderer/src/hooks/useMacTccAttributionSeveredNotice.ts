      ...
      try {
        ...
        toast.warning(
          translate(
            ...
            'macOS permissions may not reach Andes terminals'
          ),
          {
            id: SEVERED_TCC_NOTICE_ID,
            description: translate(
              'auto.hooks.useMacTccAttributionSeveredNotice.description',
              'Running Andes terminals are hosted by a daemon started by a previous Andes installation. macOS may not apply Andes’s Accessibility, Automation, or protected-file permissions to them. Restart the daemon from Manage Sessions to restore access. This will close all running Andes terminals.'
            ),
            duration: Infinity,
            action: {
              label: translate(
