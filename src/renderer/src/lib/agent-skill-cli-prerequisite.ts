    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the command line tool in PATH.'
          )
    )
    return null
  }
  ...
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Command line tool registration is unavailable'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Command line tool registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === null) {
    toast.warning(
      translate(
        ...
        'Andes could not check your Windows user PATH'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === false) {
    ...
    toast.warning(
      translate(
        ...
        'The command line tool is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            "Restart your shell or add the command line tool's directory to PATH before setup."
          )
      }
    )
  }
